...
void kernel_main(uint32_t magic, multiboot_info_t* info) {
	...
	uint32_t* a = (uint32_t*)kmalloc(2, &kernel_allocator);
	uint32_t* b = (uint32_t*)kmalloc(10, &kernel_allocator);
	uint32_t* c = (uint32_t*)kmalloc(2, &kernel_allocator);
	*a = 5;
	*b = 6;
	*c = 7;
	// write_num(*a, "a", 10);
	// write_num(*b, "b", 10);
	// write_num(*c, "c", 10);
	kfree(a, 2, &kernel_allocator);
	kfree(c, 10, &kernel_allocator);
	kfree(b, 2, &kernel_allocator);
